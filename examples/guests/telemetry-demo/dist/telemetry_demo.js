...
class StreamWritableEnd extends StreamEnd {
  ...
  _trampoline15.fnName = 'wasi:cli/exit@0.2.12#exit';
  ...
  _trampoline16.fnName = 'wasi:io/poll@0.2.12#block';
  ...
  _trampoline21.fnName = 'wasi:clocks/monotonic-clock@0.2.12#subscribeDuration';
  let exports1;
  let memory0;
  let realloc0;
  let realloc0Async;
  ...
  _trampoline26.fnName = 'wasi:io/poll@0.2.12#poll';
...
_trampoline27.fnName = 'wasi:io/streams@0.2.12#checkWrite';
...
_trampoline28.fnName = 'wasi:io/streams@0.2.12#write';
...
_trampoline29.fnName = 'wasi:io/streams@0.2.12#blockingFlush';
...
_trampoline30.fnName = 'wasi:cli/environment@0.2.12#getEnvironment';
...
_trampoline31.fnName = 'wasi:cli/terminal-stdin@0.2.12#getTerminalStdin';
...
_trampoline32.fnName = 'wasi:cli/terminal-stdout@0.2.12#getTerminalStdout';
...
_trampoline33.fnName = 'wasi:cli/terminal-stderr@0.2.12#getTerminalStderr';
let exports2;
let callback_0;
let processor010AnalyzeBatch;

async function analyzeBatch(arg0) {
  
  if (!(symbolAsyncIterator in arg0)
  && !(symbolIterator in arg0)
  && !(arg0 instanceof _PlatformReadableStream)) {
    _debugLog('[Instruction::StreamLower] object with no supported stream protocol', { arg0});
    throw new Error('unrecognized stream object (no supported stream protocol)');
  }
  
  const cstate0 = getOrCreateAsyncState(0);
  if (!cstate0) { throw new Error(`missing component state for component [0]`); }
  
  const { writeEnd: hostWriteEnd0, readEnd: readEnd0 } = cstate0.createStream({
    tableIdx: 0,
    elemMeta: {
      liftFn: _liftFlatRecord({ fieldMetas: [['sensorId', _liftFlatStringAny, 8, 4],['value', _liftFlatFloat64, 8, 8],['timestamp', _liftFlatU64, 8, 8],], size32: 24, align32: 8 }),
      lowerFn: _lowerFlatRecord({ fieldMetas: [['sensorId', _lowerFlatStringAny, 8, 4 ],['value', _lowerFlatFloat64, 8, 8 ],['timestamp', _lowerFlatU64, 8, 8 ],], size32: 24, align32: 8 }),
      payloadTypeName: 'Record(TypeRecordIndex(0))',
      isNone: false,
      isNumeric: false,
      isBorrowed: false,
      isAsyncValue: false,
  ...
  let readFn0;
  if (symbolAsyncIterator in arg0) {
    let asyncIterator = arg0[symbolAsyncIterator]();
    readFn0= () => asyncIterator.next();
    readFn0.drop = (reason) => asyncIterator.return?.(reason) ?? arg0[symbolDispose]?.();
  } else if (symbolIterator in arg0) {
    let iterator = arg0[symbolIterator]();
    readFn0= async () => iterator.next();
    readFn0.drop = (reason) => iterator.return?.(reason) ?? arg0[symbolDispose]?.();
  } else if (arg0 instanceof _PlatformReadableStream) {
    // At this point we're dealing with a readable stream that *somehow *does not*
    // implement the async iterator protocol.
    const lockedReader = arg0.getReader();
    readFn0= () => lockedReader.read();
    readFn0.drop = (reason) => lockedReader.cancel(reason).finally(() => lockedReader.releaseLock());
  }
  
  const hostInjectFn = _genStreamHostInjectFn({
    readFn: readFn0,
    hostWriteEnd: hostWriteEnd0,
    readEnd: readEnd0,
  });
  readEnd0.setHostInjectFn(hostInjectFn);
  readEnd0.setHostDropFn(readFn0.drop);
  
  const streamWaitableIdx0 = readEnd0.waitableIdx();
  
  _debugLog('[iface="local:telemetry-demo/processor@0.1.0", function="analyze-batch"][Instruction::CallWasm] enter', {
    funcName: 'analyze-batch',
    paramCount: 1,
    async: true,
    postReturn: false,
  });
  ...
  const [task, _wasm_call_currentTaskID] = createNewCurrentTask({
    ...
    isAsync: true,
    isManualAsync: false,
    entryFnName: 'processor010AnalyzeBatch',
  ...
  try {
    ret =  await  _withGlobalCurrentTaskMetaAsync({
      taskID: task.id(),
      componentIdx: task.componentIdx(),
      fn: () => processor010AnalyzeBatch(streamWaitableIdx0),
    });
  } catch (err) {
  ...
  _debugLog('[iface="local:telemetry-demo/processor@0.1.0", function="analyze-batch"][Instruction::AsyncTaskReturn]', {
    funcName: 'analyze-batch',
    paramCount: 1,
    componentIdx: 0,
    postReturn: false,
...
const trampoline0 = new WebAssembly.Suspending(streamCancelWrite.bind(null, {
  streamTableIdx: 0,
  isAsync: false,
  componentIdx: 0,
}));
...
const trampoline1 = new WebAssembly.Suspending(streamCancelRead.bind(null, {
  streamTableIdx: 0,
  isAsync: false,
  componentIdx: 0,
}));

const trampoline2 = streamDropWritable.bind(null, {
  streamTableIdx: 0,
  componentIdx: 0,
});

const trampoline3 = streamDropReadable.bind(null, {
  streamTableIdx: 0,
  componentIdx: 0,
});

const trampoline4 = streamNew.bind(null, {
  streamTableIdx: 0,
...
null,
{
  ...
  liftFns: [
  _liftFlatResult({
    caseMetas: [['ok', _liftFlatStringAny, 8, 4, 2],['err', _liftFlatVariant({
      caseMetas: [['corrupted-data', _liftFlatStringAny, 8, 4, 2],['sensor-offline', null, 0, 0, 0],['limit-exceeded', null, 0, 0, 0],],
      variantSize32: 12,
      variantAlign32: 4,
  ...
  _lowerFlatResult({
    ...
    [ 'err', _lowerFlatVariant({
      caseMetas: [[ 'corrupted-data', _lowerFlatStringAny, 8, 4, 2 ],[ 'sensor-offline', null, 0, 0, 0 ],[ 'limit-exceeded', null, 0, 0, 0 ],],
      variantSize32: 12,
      variantAlign32: 4,
      variantPayloadOffset32: 4,
      variantFlatCount: 3,
...
const trampoline23 = new WebAssembly.Suspending(streamWrite.bind(
null,
{
  componentIdx: 0,
  memoryIdx: 0,
...
const trampoline24 = new WebAssembly.Suspending(streamRead.bind(
null,
{
  componentIdx: 0,
  memoryIdx: 0,
...
const $init = (() => {
  let gen = (function* _initGenerator () {
    const module0 = fetchCompile(new URL('./telemetry_demo.core.wasm', import.meta.url));
    const module1 = base64Compile('');
    const module2 = base64Compile('');
    ...
    ({ exports: exports1 } = yield instantiateCore(yield module0, {
      $root: {
        ...
        '[waitable-set-poll]': exports0['3'],
      },
      '[export]$root': {
        '[task-cancel]': trampoline8,
      },
      '[export]local:telemetry-demo/processor@0.1.0': {
        '[async-lower][stream-read-0]analyze-batch': exports0['2'],
        '[async-lower][stream-write-0]analyze-batch': exports0['1'],
        '[stream-cancel-read-0]analyze-batch': trampoline1,
        '[stream-cancel-write-0]analyze-batch': trampoline0,
        '[stream-drop-readable-0]analyze-batch': trampoline3,
        '[stream-drop-writable-0]analyze-batch': trampoline2,
        '[stream-new-0]analyze-batch': trampoline4,
        '[task-return]analyze-batch': exports0['0'],
      },
      ...
      'wasi:cli/terminal-stderr@0.2.0': {
        'get-terminal-stderr': exports0['11'],
      },
      'wasi:cli/terminal-stdin@0.2.0': {
        'get-terminal-stdin': exports0['9'],
      },
      'wasi:cli/terminal-stdout@0.2.0': {
        'get-terminal-stdout': exports0['10'],
      },
      'wasi:clocks/monotonic-clock@0.2.0': {
        'subscribe-duration': trampoline21,
      },
      'wasi:io/error@0.2.0': {
    ...
    callback_0 = WebAssembly.promising(exports1['[callback][async-lift]local:telemetry-demo/processor@0.1.0#analyze-batch']);
    callback_0.fnName = "exports1['[callback][async-lift]local:telemetry-demo/processor@0.1.0#analyze-batch']";
    ...
    processor010AnalyzeBatch = WebAssembly.promising(exports1['[async-lift]local:telemetry-demo/processor@0.1.0#analyze-batch']);
  })();
...
export { processor010 as processor, processor010 as 'local:telemetry-demo/processor@0.1.0',  }