...
impl types::GuestDescriptor for ProxyDescriptor {
    ...
    async fn advise(
        ...
        let res: Result<(), ErrorCode> = unsafe {
            std::mem::transmute(
                self.inner
                    .advise(offset, length, unsafe { std::mem::transmute(advice) })
                    .await,
            )
        };
        res
    ...
    async fn sync_data(&self) -> Result<(), ErrorCode> {
        let res: Result<(), ErrorCode> =
            unsafe { std::mem::transmute(self.inner.sync_data().await) };
        res
    }
    ...
    async fn get_type(&self) -> Result<DescriptorType, ErrorCode> {
        let res: Result<DescriptorType, ErrorCode> =
            unsafe { std::mem::transmute(self.inner.get_type().await) };
        res
    }
    ...
    async fn set_times(
        ...
        unsafe {
            std::mem::transmute(
                self.inner
                    .set_times(
                        unsafe { std::mem::transmute(data_access_timestamp) },
                        unsafe { std::mem::transmute(data_modification_timestamp) },
                    )
                    .await,
            )
        }
    ...
    async fn create_directory_at(&self, path: String) -> Result<(), ErrorCode> {
        authorize_and_execute(
            ...
            || ErrorCode::Access,
            || async { unsafe { std::mem::transmute(self.inner.create_directory_at(path).await) } },
        )?
        .await
    }
        ...
        authorize_and_execute(
            ...
            || async {
                unsafe {
                    std::mem::transmute(
                        self.inner
                            .stat_at(unsafe { std::mem::transmute(path_flags) }, path)
                            .await,
                    )
                }
            },
            ...
            || async {
                unsafe {
                    std::mem::transmute(
                        self.inner
                            .set_times_at(
                                unsafe { std::mem::transmute(path_flags) },
                                path,
                                unsafe { std::mem::transmute(data_access_timestamp) },
                                unsafe { std::mem::transmute(data_modification_timestamp) },
                            )
            ...
            || async {
                unsafe {
                    std::mem::transmute(
                        self.inner
                            .link_at(
                                unsafe { std::mem::transmute(old_path_flags) },
                                old_path,
            ...
            || async {
                let inner = self
                    ...
                    .open_at(
                        unsafe { std::mem::transmute(path_flags) },
                        path,
                        unsafe { std::mem::transmute(open_flags) },
    ...
    async fn readlink_at(&self, path: String) -> Result<String, ErrorCode> {
        authorize_and_execute(
            &[Action::FsRead(path.clone())],
            || ErrorCode::Access,
            || async { unsafe { std::mem::transmute(self.inner.readlink_at(path).await) } },
        )?
        .await
    }
    async fn remove_directory_at(&self, path: String) -> Result<(), ErrorCode> {
        authorize_and_execute(
            &[Action::FsWrite(path.clone())],
            || ErrorCode::Access,
            || async { unsafe { std::mem::transmute(self.inner.remove_directory_at(path).await) } },
        )?
        .await
    }
    async fn rename_at(
    ...
    async fn unlink_file_at(&self, path: String) -> Result<(), ErrorCode> {
        authorize_and_execute(
            &[Action::FsWrite(path.clone())],
            || ErrorCode::Access,
            || async { unsafe { std::mem::transmute(self.inner.unlink_file_at(path).await) } },
        )?
        .await
    }
        ...
        authorize_and_execute(
            ...
            || async {
                unsafe {
                    std::mem::transmute(
                        self.inner
                            .metadata_hash_at(unsafe { std::mem::transmute(path_flags) }, path)
                            .await,
                    )
                }
            },
